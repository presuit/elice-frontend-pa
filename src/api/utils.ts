...
import { DEFAULT_ELICE_FILTER_CONDITIONS, ELICE_FILTER_CONDITION_MAP } from './constants'
import { ApiFilterConditionObj, OrgCourseFilterConditionType, OrgCourseListErrorResponses, OrgCourseListResponses } from './types'
...
export function getEliceFilterConditions(filter_conditions: string) {
  ...
  const { category, courseType, format, keyword, level, price, programmingLanguage } = JSON.parse(filter_conditions) as ApiFilterConditionObj
  ...
  if (programmingLanguage) {
    const data = convertFilterConditions('programmingLanguage', programmingLanguage)
    filterConditions.$and.push(data)
  }
...
function getEliceFilterCondition(key: OrgCourseFilterConditionType, value: string) {
  const container = []
...
export function convertFilterConditions(key: OrgCourseFilterConditionType, value: string | string[]) {
...
export function isOrgCourseErrorResponses(data: OrgCourseListResponses | OrgCourseListErrorResponses): data is OrgCourseListErrorResponses {
...
export function withCorsHandler(fn: (req: NextApiRequest, res: NextApiResponse) => Promise<void>) {
